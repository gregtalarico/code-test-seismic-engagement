...
function monthlyCharge(yearMonth, subscription, users) {
  ...
  // Retrieve users that match the subscription customer id
  const customerId = subscription.customerId;
  const customerUsers = users.filter((user) => user.customerId == customerId);
  
  // first and last day of month
  const firstDay = firstDayOfMonth(new Date(yearMonth + '-1'));
  const lastDay = lastDayOfMonth(new Date(yearMonth + '-1'));
  
  // rate per day
  const daysInMonth = lastDay.getDate();
  const ratePerDay = (subscription.monthlyPriceInCents / daysInMonth).toFixed(2)
  
  const dayArray = new Array(daysInMonth)
  dayArray.forEach((day, index) => {
    const currentDate = new Date(`${yearMonth}-#{index+1}`)
    const activeUserCount = customerUsers.filter((user) => {
      user.deactivatedOn >= currentDate && user.activatedOn <= currentDate
    }).length
    
    totalAmount += activeUserCount * ratePerDay;
  })
